...
for (let i = 6; i <= 100; i++) {
  ...
  additionalJobs.push({
    ...
    location: ['Remote', 'New York, NY', 'San Francisco, CA', 'Austin, TX', 'Chicago, IL'][Math.floor(Math.random() * 5)],
    type: ['full-time', 'part-time', 'contract', 'remote'][Math.floor(Math.random() * 4)] as any,
    salary: `$${60 + Math.floor(Math.random() * 80)},000 - $${100 + Math.floor(Math.random() * 80)},000`,
